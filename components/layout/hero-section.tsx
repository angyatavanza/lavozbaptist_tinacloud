...
interface SectionHeroProps extends React.HTMLProps<HTMLElement> {
  background?: string;
  children: ReactNode;
}

export const SectionHero: React.FC<SectionHeroProps> = ({ className, children, background, ...props }) => {
  return (
    <div className={cn("mx-auto max-w-full min-h-screen", background || "bg-default")}>
      <section
        className={cn("mx-auto max-w-full min-h-screen", className)}
        {...props}
      >
        {children}
      </section>
...
export const sectionheroBlockSchemaField = {
  type: "string",
  label: "Background",
  name: "background",
